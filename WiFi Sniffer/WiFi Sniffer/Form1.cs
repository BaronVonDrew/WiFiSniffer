    ...
    public partial class Form1 : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                label3.Text = "# of Connections: " + ns.Count();
                label5.Text = "Established: " + ns.Established();
            }
            catch (Exception) { }
        }
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (row >= 0)
            {
                string addr = dataGridView1.Rows[row].Cells[1].Value.ToString();
                Thread t = new Thread(() => NetStats.GetHostName(row));
                t.Start();

                while (t.IsAlive) {  }

                textBox1.Text = "(" + addr + ") => " + NetStats.HOST;
            }
        }
    }
}
